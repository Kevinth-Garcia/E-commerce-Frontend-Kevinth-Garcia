...
export default function AdminUsers() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await api.get("/admin/users"); // ruta
  ...
  const toggleAdmin = async (u) => {
    try {
      ...
      const res = await api.put(`/admin/users/${u._id}`, { isAdmin: next }); // PUT
  ...
  const doDelete = async () => {
    ...
    try {
      await api.delete(`/admin/users${confirmDelete._id}`); // DELETE
